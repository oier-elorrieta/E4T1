...
public class PlaylistAbestiakBista extends JFrame implements Inabegazioa {
	...
	public PlaylistAbestiakBista(Bezero bz, Playlist playlist) {
		...
		for (int i = 0; i < lista.getAbestiList().size(); i++) {
			playlistModel.addElement(lista.getAbestiList().get(i).getAbestiIzena() + " - " + erreprodukzioak + " - "
					+ abeslaria.get(i) + " - " + lista.getAbestiList().get(i).getIraupena());
		}
		...
		btnExportatu.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (listAbestiPlaylist.getSelectedIndex() == -1) {
					JOptionPane.showMessageDialog(null, "Ez duzu ezer aukeratu kompartitzeko");
				} else {
					PlaylistFuntzioak.AbestiakExportatu(lista, erreprodukzioak, abeslaria,
							listAbestiPlaylist.getSelectedIndex());
					btnExportatu.setEnabled(false);
					btnEzabatu.setEnabled(false);
				}
