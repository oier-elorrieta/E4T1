...
public class PlaylistAbestiakBista extends JFrame implements Inabegazioa{
    ...
    public PlaylistAbestiakBista(Bezero bz, Playlist playlist) {
        ...
        Playlist lista = NirePlaylistDao.AteraPlaylistAbestiak(playlist);
         erreprodukzioak = NirePlaylistDao.ErreprodukzioakAtera(playlist);
		...
		for (int i = 0; i < lista.getAbestiList().size(); i++) {
			playlistModel.addElement(lista.getAbestiList().get(i).getAbestiIzena() + " - "+ erreprodukzioak.get(i) + " - " + abeslaria.get(i) + " - " + lista.getAbestiList().get(i).getIraupena());
		}
